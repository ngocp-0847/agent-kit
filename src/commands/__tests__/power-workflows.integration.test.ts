...
import { PowerCacheManager, PowerManagerImpl } from "../../utils/power";
...
describe("End-to-End Power Workflows", () => {
  ...
  describe("Complete Power Installation Workflow", () => {
    it("should complete full installation workflow from registry to working Power", async () => {
      ...
      expect(mcpConfig.mcpServers["registry-scanner"].args).toEqual([
        "./servers/registry-scanner.js",
      ]);

      // Step 4: Verify steering files
      const steeringFiles = ["registry-getting-started.md", "registry-advanced-usage.md"];
      ...
      expect(installed.powers["registry-power"].components.mcpServers).toContain(
        "registry-scanner",
      );
      expect(installed.powers["registry-power"].components.steeringFiles).toContain(
  ...
  describe("Power Update Workflow", () => {
    it("should update Power while preserving user customizations", async () => {
      ...
      expect(updatedMcpConfig.mcpServers["updatable-power-scanner"].env.CUSTOM_VAR).toBe(
        "user-value",
      );
  ...
  describe("Error Handling", () => {
    it("should fail gracefully on invalid Power structure", async () => {
      ...
      await expect(
        powerManager.installLocalPower(invalidPowerDir, "invalid-power"),
      ).rejects.toThrow();
...
`;
  ...
  writeFileSync(
    join(steeringDir, "registry-getting-started.md"),
    "# Getting Started\n\nRegistry Power getting started guide.",
  );
  writeFileSync(
    join(steeringDir, "registry-advanced-usage.md"),
  ...
  writeFileSync(
    join(serversDir, "registry-scanner.js"),
...
function createPowerWithRequirements(powerDir: string) {
  ...
  writeFileSync(
    join(powerDir, "POWER.md"),
    "# Power with Requirements\n\nThis Power has specific requirements.",
  );
  ...
  mkdirSync(steeringDir, { recursive: true });
  writeFileSync(
    join(steeringDir, "setup-requirements.md"),
    "# Setup Requirements\n\nThis Power requires Node.js 18+ and specific dependencies.",
  );
}

function createTestPowerStructure(powerDir: string, powerName: string, version = "1.0.0") {
  mkdirSync(powerDir, { recursive: true });
  ...
  mkdirSync(steeringDir, { recursive: true });
  writeFileSync(
    join(steeringDir, `${powerName}-getting-started.md`),
